...
def test_invalid_grid_spacing():
    """
    Check if a mesh voxelizer class is able to detect invalid grid spacing when bounds are correct.
    """
    voxel_size = 1
    broken_spacing = np.array([0, 1.5, 2.5, 3], dtype=float)
    valid_grid = np.linspace(0, 3, 4)

    with pytest.raises(ValueError):
        MeshVoxelizer(voxel_size, broken_spacing, valid_grid, valid_grid)


def test_fast_winding_number_fallback(monkeypatch):
    """
    ...
    try:
        ...
        if original_module is not None:
            sys.modules["magnet_pinn.preprocessing.voxelizing_mesh"] = original_module
