...
def check_cropped_shapes(result):
    ...
    assert result.positions.shape == (3, 10, 10, 10)
    assert result.phase.shape == (8,)
    assert result.mask.shape == (8,)
    assert result.coils.shape == (8, 10, 10, 10)
...
def check_default_transform_resulting_values(result, item):
    ...
    assert np.equal(result.field, np.sum(item.field, axis=2)).all()

    coils_num = item.coils.shape[0]
