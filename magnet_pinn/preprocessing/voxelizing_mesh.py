...
class MeshVoxelizer:
    ...
    def __init__(
        ...
        x_grid, y_grid, z_grid = np.meshgrid(
            x_unique, y_unique, z_unique, indexing="ij"
        )
        self.points = np.stack([x_grid, y_grid, z_grid], axis=-1)
        self.points = np.ascontiguousarray(
            einops.rearrange(self.points, "x y z d -> (x y z) d")
        )
    ...
    def __validate_input(self, grid: npt.NDArray[np.float64], axis: str):
        ...
        if not np.equal(grid, supposed_grid).all():
            raise ValueError(
                f"Invalid {axis} grid {grid} for the {self.voxel_size} vixel size."
            )
    ...
    def process_mesh(self, mesh: Trimesh) -> np.array:
        ...
        mask = np.logical_and(~np.isclose(winding_number, 0.5), winding_number > 0.5)
        mask = np.ascontiguousarray(
            einops.rearrange(
                mask, "(x y z) -> x y z", x=len(self.x), y=len(self.y), z=len(self.z)
            )
        ).astype(np.uint8)
        return mask
