...
def validate_arguments(args: Namespace) -> None:
    ...
    if hasattr(args, "blob_radius_decrease") and (
        args.blob_radius_decrease <= 0 or args.blob_radius_decrease >= 1
    ):
    ...
    if hasattr(args, "relative_tube_max_radius") and (
        args.relative_tube_max_radius <= 0 or args.relative_tube_max_radius >= 1
    ):
        raise ValueError("relative_tube_max_radius must be in range (0, 1)")

    if (
        hasattr(args, "relative_tube_min_radius")
        and hasattr(args, "relative_tube_max_radius")
        and args.relative_tube_min_radius >= args.relative_tube_max_radius
    ):
        raise ValueError(
            "relative_tube_min_radius must be less than relative_tube_max_radius"
        )
...
def _validate_custom_arguments(args: Namespace):
    ...
    if not args.stl_mesh_path.suffix.lower() == ".stl":
        raise ValueError(
            f"STL mesh file must have .stl extension: {args.stl_mesh_path}"
        )
