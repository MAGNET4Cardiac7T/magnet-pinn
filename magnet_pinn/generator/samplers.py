...
class BlobSampler:
    ...
    def check_points_distance(self, points: np.ndarray, min_distance: float) -> bool:
        ...
        distances = distances + np.eye(len(points)) * 1e10
    ...
    def _find_valid_positions_progressive(
        self,
...
class MeshBlobSampler:
    ...
    def __init__(self, child_radius: float, sample_children_only_inside: bool = False):
        """Initialize mesh blob sampler.

        Parameters
        ----------
        child_radius : float
            Radius of child blobs. Must be positive.
        sample_children_only_inside : bool, optional
            If True, ensure blobs are fully inside mesh. Default is False.

        Raises
        ------
        ValueError
            If child_radius is not positive.
        """
        if child_radius <= 0:
    ...
    def sample_children_blobs(
        ...
        effective_radii = np.array([blob.effective_radius for blob in placed_blobs])
...
class MeshTubeSampler:
    ...
    def __init__(
        ...
        ----------
        tube_max_radius : float
            Maximum tube radius. Must be positive.
        tube_min_radius : float
            Minimum tube radius. Must be positive and < max_radius.
        parent_radius : float, optional
            Parent structure radius. Default is 250.

        Raises
        ------
        ValueError
            If radii are invalid or min >= max.
        """
        if tube_max_radius <= 0:
            raise ValueError("tube_max_radius must be positive")
        if tube_min_radius <= 0:
