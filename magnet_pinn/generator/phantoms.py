...
from .samplers import BlobSampler, TubeSampler, MeshBlobSampler, MeshTubeSampler
...
class Tissue(Phantom):
    ...
    def __init__(
        ...
        if initial_blob_center_extent is None or (
            ...
            raise ValueError(
                "initial_blob_center_extent must be a 2d array-like structure with coordinate ranges, first X, then Y, then Z dimensions"
            )
...
class CustomPhantom(Phantom):
    ...
    def __init__(
        self,
        stl_mesh_path: str,
        num_children_blobs: int = 3,
        blob_radius_decrease_per_level: float = 0.3,
        num_tubes: int = 5,
        relative_tube_max_radius: float = 0.1,
        relative_tube_min_radius: float = 0.01,
        sample_children_only_inside: bool = False,
    ):
        """Initialize custom phantom with mesh and generation parameters.

        Parameters
        ----------
        stl_mesh_path : str
            Path to STL mesh file for parent structure.
        num_children_blobs : int, optional
            Number of child blobs to generate. Default is 3.
        blob_radius_decrease_per_level : float, optional
            Multiplier for child radius relative to parent. Default is 0.3.
        num_tubes : int, optional
            Number of tubes to generate. Default is 5.
        relative_tube_max_radius : float, optional
            Maximum tube radius as fraction of parent radius. Default is 0.1.
        relative_tube_min_radius : float, optional
            Minimum tube radius as fraction of parent radius. Default is 0.01.
        sample_children_only_inside : bool, optional
            If True, ensure children are fully inside parent. Default is False.
        """
    ...
    def generate(self, seed: int = None, child_blobs_batch_size: int = 1000000) -> StructurePhantom:
        ...
        child_blobs_batch_size : int, optional
            Batch size for sampling child positions. Default is 1000000.

        Returns
        -------
        StructurePhantom
            Generated phantom with parent, children, and tube structures.
        """
