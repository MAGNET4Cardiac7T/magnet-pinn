...
def parse_args():
    parser = argparse.ArgumentParser(description="Generate meshes with a custom parent mesh and tubes/blobs inside.")
    parser.add_argument('--seed', type=int, default=42, help='Random seed for reproducibility')
    parser.add_argument('--output_dir', type=str, default='./gen_data/raw/custom_mesh',
                        help='Directory to save the generated meshes and properties')
    parser.add_argument('--stl_mesh_path', type=str, default='./phantom.stl',
                        help='Path to the STL file of the custom parent mesh')
    parser.add_argument('--num_children_blobs', type=int, default=3,
                        help='Number of child blobs to generate')
    parser.add_argument('--num_tubes', type=int, default=10,
                        help='Number of tubes to generate')
...
seeds = seed_sampler.integers(0, 100000, size=(args.num_children_blobs + 1, args.num_tubes + 1))
...
for num_children_blobs in range(args.num_children_blobs+1):
    for num_tubes in range(args.num_tubes+1):
        ...
        print(f"Generated meshes with {num_children_blobs} children blobs and {num_tubes} tubes.")
