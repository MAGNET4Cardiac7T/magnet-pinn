...
class Iterator:
    def __init__(self, path):
        ...
        self.iterator = MagnetGridIterator(
            path, transforms=augmentation, num_samples=10
        )
    ...
    def __iter__(self):
        for batch in self.iterator:
            ...
            target = einops.rearrange(
                batch["field"], "he reim xyz ... -> (he reim xyz) ..."
            )
            yield {
                "input": input,
                "target": target,
...
input_normalizer.save_as_json(
    "data/processed/train/grid_voxel_size_4_data_type_float32/normalization/input_normalization.json"
)

target_normalizer = StandardNormalizer()
target_normalizer.fit_params(iterator, key="target", axis=0)
target_normalizer.save_as_json(
    "data/processed/train/grid_voxel_size_4_data_type_float32/normalization/target_normalization.json"
)
