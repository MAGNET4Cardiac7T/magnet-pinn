...
class MAGNETPINN(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        properties, phase, field, subject_mask = (
            batch["input"],
            batch["coils"],
            batch["field"],
            batch["subject"],
...
augmentation = Compose([Crop(crop_size=(100, 100, 100)), GridPhaseShift(num_coils=8)])

iterator = MagnetGridIterator(BASE_DIR, transforms=augmentation, num_samples=100)

dataloader = DataLoader(
    iterator, batch_size=4, num_workers=16, worker_init_fn=worker_init_fn
)
...
target_normalizer = StandardNormalizer.load_from_json(
    f"{BASE_DIR}/normalization/target_normalization.json"
)
input_normalizer = StandardNormalizer.load_from_json(
    f"{BASE_DIR}/normalization/input_normalization.json"
)
